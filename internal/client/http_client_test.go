...
func TestHTTPClient_DoTypedJSONRequest(t *testing.T) {
	// Mock server
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("Content-Type", "application/json")
		w.WriteHeader(http.StatusOK)
		json.NewEncoder(w).Encode(TypedResponseWrapper[string]{
			Code:    0,
			Message: "success",
			Data:    "test data",
		})
	...
	ctx := context.Background()
	wrapper, err := DoTypedJSONRequest[string](client, ctx, req)

	if err != nil {
		t.Fatalf("DoTypedJSONRequest failed: %v", err)
	}

	if wrapper.Code != 0 {
		t.Errorf("Expected code 0, got %d", wrapper.Code)
	}

	if wrapper.Message != "success" {
		t.Errorf("Expected message 'success', got '%s'", wrapper.Message)
	}

	if wrapper.Data != "test data" {
		t.Errorf("Expected data 'test data', got '%v'", wrapper.Data)
	}
}

func TestHTTPClient_DoTypedJSONRequest_InvalidJSON(t *testing.T) {
	...
	ctx := context.Background()
	_, err := DoTypedJSONRequest[string](client, ctx, req)
...
func TestDoTypedJSONRequest(t *testing.T) {
	// Mock server
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("Content-Type", "application/json")
		w.WriteHeader(http.StatusOK)
...
func TestDoTypedJSONRequest_WithStruct(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("Content-Type", "application/json")
		w.WriteHeader(http.StatusOK)
		json.NewEncoder(w).Encode(TypedResponseWrapper[TestData]{
			Code:    0,
			Message: "success",
			Data: TestData{
				Name:  "test",
	...
	ctx := context.Background()
	wrapper, err := DoTypedJSONRequest[TestData](client, ctx, req)
	...
	if wrapper.Data.Name != "test" {
		t.Errorf("Expected name 'test', got '%s'", wrapper.Data.Name)
	}

	if wrapper.Data.Value != 42 {
		t.Errorf("Expected value 42, got %d", wrapper.Data.Value)
	}
}

func TestDoTypedRequest(t *testing.T) {
	type TestData struct {
		Name  string `json:"name"`
		Value int    `json:"value"`
	}
	...
	ctx := context.Background()
	data, err := DoTypedRequest[TestData](client, ctx, req)

	if err != nil {
		t.Fatalf("DoTypedRequest failed: %v", err)
	}
...
func TestDoTypedJSONRequest_HTTPError(t *testing.T) {
	// Mock server that returns error
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		http.Error(w, "Internal Server Error", http.StatusInternalServerError)
	}))
...
func TestDoTypedJSONRequest_InvalidJSON(t *testing.T) {
	// Mock server that returns invalid JSON
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("Content-Type", "application/json")
		w.WriteHeader(http.StatusOK)
